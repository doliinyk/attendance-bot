...
import static ods.attendancebot.handlers.TimeEventHandler.findElementByCssSelector;
import static ods.attendancebot.handlers.TimeEventHandler.findElementsByCssSelector;
...
public class CollectHandler {
	public static List<WebElement> collectEventsFromMainWindow() {
		...
		return events.filter(TimeEventHandler::filterAttendancesDayAndTime)
				.collect(toList());
	}
	...
	private static Stream<WebElement> collectActivitiesByCssSelector(String cssSelector) {
		return findElementsByCssSelector(cssSelector).stream()
				.filter(TimeEventHandler::filterAttendancesFromEvents);
	}
